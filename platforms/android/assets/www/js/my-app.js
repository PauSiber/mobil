            ...
            if(yanit.status_code == 200)
            {
                ...
                $.each(yanit.gelir_gider, function (index, value) {
                    ...
                    $('.anasayfa').append('' +
                        ...
                        '</div> ' +
                        '<div class="item-subtitle"><span class="'+tur+'" style="margin-right: 25px;"> - Hesap : '+value.oda+'</span></div> ' +
                        '</div> ' +
                        '</a> ' +
        ...
        success: function (yanit) {
            ...
            if (yanit.status_code == 200) {
                $.each(yanit.odalar, function (index, value) {
                    ...
                    $('.odalar').append('' +
                        '<li class="accordion-item swipeout" id="oda_key_'+value.oda_key+'" > ' +
                        '<a href="#" class="item-content swipeout-content item-link"> ' +
                        '<div class="item-media"> ' +
                        ...
                        '</div> ' +
                        '<div class="item-subtitle"><span class="fa fa-users" style="margin-right: 20px;">'+kullanici_str+'</span></div> ' +
                        '</div> ' +
                        '</a> ' +
                        '<div class="accordion-item-content"> ' +
                        '<div class="content-block"> ' +
                        ...
                        '</div> ' +
                        '<div class="swipeout-actions-right"> ' +
                        '<a href="#" class="swipeout-overswipe oda_sil" data-oda-key="'+value.oda_key+'" data-confirm="Odadan çıkmak istediğinize emin misiniz?" data-confirm-title="Geri dönüşünüz olmayabilir.."><span class="fa fa-trash-o"></span></a> ' +
                        '</div> ' +
                        '</div>' +
                        '</li>' +
                        '')
...
$(document).on('click','.gelirGiderKaydetButon', function () {
    ...
    var tur = $('.tur_selectbox option:selected').val();
   ...
   // alert(window.localStorage.getItem('tur'));
    ...
    var data = 'access_token='+window.localStorage.getItem('access_token')+'&tur='+tur+'&key='+oda_key+'&kategori='+kategori+'&tutar='+tutar+'&aciklama='+aciklama;
            ...
            if (yanit.status_code == 200) {
                $.each(yanit.odalar, function (index, value) {
                    ...
                    $('.odalar').append('' +
                        ...
                        '</div> ' +
                        '<div class="swipeout-actions-right"> ' +
                        '<a href="#" class=" swipeout-overswipe oda_sil" data-oda-key="'+value.oda_key+'" ><span class="fa fa-trash-o"></span></a> ' +
                        '</div> ' +
                        '</div>' +
                        '</li>' +
                        '')
...
$(document).on('click','.oda_sil',function () {
    var oda_key = $(this).attr('data-oda-key');
    //alert(oda_key);
    myApp.confirm('Odadan çıkmak istediğine emin misin?','Gider.XYZ',function () {
        //myApp.alert('Evet dedim');

        $('#oda_key_'+oda_key).fadeOut("fast");

        $.ajax({
            url: base_url + 'oda/cikis',
            data: 'access_token=' + token+'&key='+oda_key,
            type: 'post',
            dataType: 'json',
            success: function (yanit) {
                //alert(yanit.status_code);
...
$(document).on('click','.sil',function () {
    ...
    myApp.confirm('Silmek istediğine emin misin?','Gider.XYZ',function () {
        //myApp.alert('Evet dedim');
...
$(document).on('click','.onayla',function () {
    var alacak_verecek_id = $(this).attr('data-alacak');
    myApp.confirm('Onaylamak istediğine emin misin?','Gider.XYZ',function () {
        //myApp.alert('Evet dedim');
        $('#alacak_verecek_id_'+alacak_verecek_id).fadeOut("fast");
                ...
                success: function (yanit) {
                    if(yanit.status_code == 200)
                    {
                        ...
                        $('.odalar').append('' +
                            ...
                            '</div> ' +
                            '<div class="swipeout-actions-right"> ' +
                            '<a href="#" class="swipeout-delete swipeout-overswipe" data-oda-key="'+yanit.oda_key+'" data-oda-key="'+yanit.oda_key+'" data-confirm="Odadan çıkmak istediğinize emin misiniz?" data-confirm-title="Geri dönüşünüz olmayabilir.."><span class="fa fa-trash-o"></span></a> ' +
                            '</div> ' +
                            '</div>' +
                            '</li>' +
                            '')
